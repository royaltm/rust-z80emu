...
macro_rules! instr {
    (@# @extended | break $main:tt
    #@ $deb:expr; $prefix:ident, $flags:ident, $pc:ident, $cpu:ident, $control:ident, $tsc:ident; [$_:expr, $code:expr]) => {
        {
            // pc+1:4
            let code = fetch_next_opcode!($cpu, $control, $pc, $tsc);
            match_instruction!(ED [code] $deb; $prefix, $flags, $pc, $cpu, $control, $tsc; break $main)
        }
    };
    (@# @prefix = $new:ident
    #@ $deb:expr; $prefix:ident, $flags:ident, $pc:ident, $cpu:ident, $control:ident, $tsc:ident; [$_:expr, $code:expr]) => {
    ...
    (@# @prefix = $new:ident; break $label:tt
    #@ $deb:expr; $prefix:ident, $flags:ident, $pc:ident, $cpu:ident, $control:ident, $tsc:ident; [$_:expr, $code:expr]) => {
        {
    ...
    (@#     NOP *
    #@ $deb:expr; $prefix:ident, $flags:ident, $pc:ident, $cpu:ident, $control:ident, $tsc:ident; [$code0:expr, $code1:expr]) => {
    ...
    (@#     LD r,r | LD (HL),r | LD r,(HL) | HALT break $main:tt
    #@ $deb:expr; $prefix:ident, $flags:ident, $pc:ident, $cpu:ident, $control:ident, $tsc:ident; [$_:expr, $code:expr]) => {
        {
            debug_assert_eq!($prefix, Prefix::None);
    ...
    (@#     LD q,q | LD (ii+d),r | LD r,(ii+d) | continue $label:tt
    #@ $deb:expr; $prefix:ident, $flags:ident, $pc:ident, $cpu:ident, $control:ident, $tsc:ident; [$_:expr, $code:expr]) => {
        {
            debug_assert_ne!($prefix, Prefix::None);
            match Reg8::tuple_from_b5_3_and_b2_0($code) {
    ...
    (@#     @ops A,q | @ops A,(ii+d) | continue $label:tt
    #@ $deb:expr; $prefix:ident, $flags:ident, $pc:ident, $cpu:ident, $control:ident, $tsc:ident; [$_:expr, $code:expr]) => {
        {
            debug_assert_ne!($prefix, Prefix::None);
            let op = Ops8::from($code);
    ...
    (@#     @rot|BIT|RES|SET r|(HL)
    #@ $deb:expr; $prefix:ident, $flags:ident, $pc:ident, $cpu:ident, $control:ident, $tsc:ident; [$_:expr, $code0:expr]) => {
        { // pc+1:4
            debug_assert_eq!($prefix, Prefix::None);
