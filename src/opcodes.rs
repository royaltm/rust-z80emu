...
macro_rules! match_instruction {
    ...
    $repeat:tt) => { instruction_dispatch! { $deb; $prefix, $flags, $pc, $cpu, $control, $tsc;
        match (__,$code) {
            ...
            0xBF..=0xCA|0xCC..=0xDC|0xDE..=0xE0|0xE2|0xE4|0xE6..=0xE8|0xEA..=0xF8|
            0xFA..=0xFC|0xFE|0xFF => {  @setnoprefix continue $repeat                         }
            0x40..=0x7F => {            LD q,q | LD (ii+d),r | LD r,(ii+d) | continue $repeat } // 0b01_rrr_rrr
            0x80..=0xBF => {            @ops A, q | @ops A, (ii+d) | continue $repeat         } // 0b10_ari_rrr
            ...
            0x2B => {                   DEC  ii                                               }
            0xE9 => {                   JP   (ii)                                             }
            0xDD => {                   @prefix = Xdd; break $repeat                          }
            0xFD => {                   @prefix = Yfd; break $repeat                          }
        }
    }};
...
/// Takes the byte in $code and according to the $prefix dispatches the op-codes via opcodes::match_instruction!.
    ...
    ($deb:expr; $prefix:ident, $flags:ident, $pc:ident, $cpu:ident, $control:ident, $tsc:ident; break $main:tt) => {
        {
            ...
            execute_instruction!([code] $deb; $prefix, $flags, $pc, $cpu, $control, $tsc; break $main)
        }
    };
}
